//Link: https://karatstudio.karat.io/interview/7eb36450-4047-4da8-ba31-22a261e86554/candidate?um=true
...
class RunCollection {
  ...
  public static List<Run> runs;  // the Run objects for this particular course
  ...
  public static int bestOfBests(){
    ...
    for(int i =0; i<runs.size(); i++){
      ...
      for(int j=0; j<3; j++){
          // System.out.println(runs.get(i).obstacleTimes.get(j));
          min = Math.min(runs.get(i).obstacleTimes.get(j), min);
      }
      sum += min;
    }
    return sum;
...
public class Solution {
  ...
  public static void testRunCollection() {
    ...
    assert runCollection.bestOfBests() == 10 : 
      "bestOfBests should be 10, was " + runCollection.personalBest();
  }
}